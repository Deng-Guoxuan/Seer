...
World2::World2(QWidget *parent) : QMainWindow(parent)
{
    ...
    this->_winLabel->setGeometry(3.5*PIX,2*PIX,9*PIX,4*PIX);
    this->_winLabel->setFont(QFont("微软雅黑",90,70));//字体，大小，粗细
    ...
    this->_loseLabel->setGeometry(3.5*PIX,2*PIX,9*PIX,4*PIX);
    this->_loseLabel->setFont(QFont("微软雅黑",90,70));//字体，大小，粗细
    this->_loseLabel->setStyleSheet("color:black");//背景颜色，字体颜色
    ...
    playMusic->setGeometry(2*PIX,0,PIX,PIX);//位置
    ...
    stopMusic->setGeometry(2*PIX,0,PIX,PIX);//位置
    ...
    connect(timer1,&QTimer::timeout,[=](){                          //设置路径点
        Point* path1[]={new Point(3*PIX,4*PIX),new Point(15*PIX,4*PIX),new Point(15*PIX,8*PIX),new Point(5*PIX,8*PIX),
                        new Point(5*PIX,4*PIX),new Point(11*PIX,4*PIX),new Point(11*PIX,8*PIX),new Point(1*PIX,8*PIX),
                        new Point(1*PIX,6*PIX),
                        new Point(this->_base)
                       };//最后的路径点设为家

        Point* path2[]={new Point(3*PIX,4*PIX),new Point(13*PIX,4*PIX),new Point(13*PIX,8*PIX),new Point(3*PIX,8*PIX),
                        new Point(3*PIX,4*PIX),new Point(11*PIX,4*PIX),new Point(11*PIX,8*PIX),new Point(1*PIX,8*PIX),
                        new Point(1*PIX,6*PIX),
                        new Point(this->_base)
                       };//路线2

        Point entrance[]={Point(0, 4*PIX),Point(0.25*PIX,4*PIX),Point(0.5*PIX,4*PIX),Point(0.75*PIX,4*PIX)};//入口位置(增加入口以便错开海盗)
...
//界面所有海盗找目标
void World2::allPirateFindTarget(){
...
//界面所有精灵找目标
void World2::allSpiritsFindTarget(){
...
void World2::redBulletEffect(Point &p,int damage,int type){
    int splashRange=0;
    if(type==2){
        splashRange=80;
    }
...
void World2::blueBulletEffect(Point &p, double percentage,int type){
    int waveRange=0;
    if(type==3){
        waveRange=100;
    }
...
//绘图事件
void World2::paintEvent(QPaintEvent *){
    QPainter painter(this);//画家类

    DrawMap2(painter);//画出地图
...
void World2::DrawLighting(QPainter &painter){
    for(auto pirate:this->_pirateVector){
...
void World2::DrawSpiritLifeBar(QPainter &painter){
...
void World2::DrawPirateLifeBar(QPainter &painter){
...
void World2::DrawSplash(QPainter &painter){
...
void World2::DrawWave(QPainter &painter){
...
void World2::DrawAddLife(QPainter &painter){
    for(auto spirit:this->_spiritsVector){
...
//该函数只执行一次,地图初始化后不再执行
void World2::setMap2(){
    Point capsules[]={Point(0*PIX,5*PIX),Point(0*PIX,7*PIX),Point(0*PIX,8*PIX),Point(0*PIX,9*PIX),
                      Point(1*PIX,5*PIX),Point(1*PIX,9*PIX),
                      Point(2*PIX,3*PIX),Point(2*PIX,5*PIX),Point(2*PIX,6*PIX),Point(2*PIX,7*PIX),Point(2*PIX,9*PIX),
                      Point(3*PIX,3*PIX),Point(3*PIX,9*PIX),
                      Point(4*PIX,3*PIX),Point(4*PIX,5*PIX),Point(4*PIX,6*PIX),Point(4*PIX,7*PIX),Point(4*PIX,9*PIX),
                      Point(5*PIX,3*PIX),Point(5*PIX,9*PIX),
                      Point(6*PIX,3*PIX),Point(6*PIX,5*PIX),Point(6*PIX,6*PIX),Point(6*PIX,7*PIX),Point(6*PIX,9*PIX),
                      Point(7*PIX,3*PIX),Point(7*PIX,5*PIX),Point(7*PIX,6*PIX),Point(7*PIX,7*PIX),Point(7*PIX,9*PIX),
                      Point(8*PIX,3*PIX),Point(8*PIX,5*PIX),Point(8*PIX,6*PIX),Point(8*PIX,7*PIX),Point(8*PIX,9*PIX),
                      Point(9*PIX,3*PIX),Point(9*PIX,5*PIX),Point(9*PIX,6*PIX),Point(9*PIX,7*PIX),Point(9*PIX,9*PIX),
                      Point(10*PIX,3*PIX),Point(10*PIX,5*PIX),Point(10*PIX,6*PIX),Point(10*PIX,7*PIX),Point(10*PIX,9*PIX),
                      Point(11*PIX,3*PIX),Point(11*PIX,9*PIX),
                      Point(12*PIX,3*PIX),Point(12*PIX,5*PIX),Point(12*PIX,6*PIX),Point(12*PIX,7*PIX),Point(12*PIX,9*PIX),
                      Point(13*PIX,3*PIX),Point(13*PIX,9*PIX),
                      Point(14*PIX,3*PIX),Point(14*PIX,5*PIX),Point(14*PIX,6*PIX),Point(14*PIX,7*PIX),Point(14*PIX,9*PIX),
                      Point(15*PIX,3*PIX),Point(15*PIX,9*PIX),
                      };    //精灵屋位置
...
//画出地图，包括：精灵屋位置、基地位置
void World2::DrawMap2(QPainter &painter){
    painter.drawPixmap(0,0,1152,640,QPixmap(":/Image/pictures/Map2.png"));//地图
    painter.drawPixmap(0,6*PIX,64,64,QPixmap(":/Image/pictures/base.png"));//基地
...
void World2::addPirate(int pirateType, Point **path, int pathLength,Point& startPoint){
    switch (pirateType) {
    case 1:
...
void World2::setPiratesWave(Point **path1, Point **path2, Point *entrance, int*pathLengths){
    ...
    else if(this->_count>=48&& this->_count<=54){
        this->setPirateBlank(3000);
        addPirate(4,ways[0],pathLengths[0],entrance[0]);
        addPirate(4,ways[1],pathLengths[1],entrance[2]);
    }
    else if(this->_count>=55&& this->_count<=60){
        this->setPirateBlank(5000);
        addPirate(6,ways[0],pathLengths[0],entrance[0]);
        addPirate(6,ways[1],pathLengths[1],entrance[2]);
    }
    else if(this->_count>=61 && this->_count<=67){
        this->setPirateBlank(4000);
        addPirate(1,ways[0],pathLengths[0],entrance[0]);
        addPirate(2,ways[0],pathLengths[0],entrance[2]);
        addPirate(1,ways[1],pathLengths[1],entrance[1]);
...
//画海盗及其子弹
void World2::DrawPirate(QPainter &painter){
...
//画精灵及其子弹
void World2::DrawSpirits(QPainter &painter){
...
void World2::DrawEnvovleBox(QPainter &painter){
...
void World2::DrawSelectionBox(QPainter &painter){
    //显示选择框
...
//将鼠标点击转化为格子点击
bool World2::clickThisBlock(int mouseX, int mouseY, int blockX, int blockY){
...
void World2::setCapsuleOccupied(const Point &p, const int k){
    for(auto capsule:this->_capsuleVector){
...
//鼠标点击事件
void World2::mousePressEvent(QMouseEvent *ev){
...
void World2::ClickCapsule(QMouseEvent *&ev){
    //判断塔坑的点击
...
void World2::ClickEnvolveBox(QMouseEvent *&ev){
    //判断升级框2个子按钮的点击
...
void World2::ClickSelectionBox(QMouseEvent *&ev){
    //判断选择框6个子按钮的点击
...
bool World2::inRange(int distance, int range){
    if(distance<=range){
        return true;
    }
    else return false;
...
bool World2::isBingo(Point &bulletP, Point &targetP){
...
void World2::setPirateBlank(const int blank){
    this->_pirateBlank=blank;
}

int World2::getPirateBlank()const{
    return this->_pirateBlank;
}

void World2::eraseSpirit(Point &p){
...
void World2::returnMoney(const int type){
    int money=0;
    switch (type) {
    case 1:
    case 2:
